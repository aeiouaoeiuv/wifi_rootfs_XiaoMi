...
function index()
    ...
    entry({"api", "xqnetwork", "check_wan_type"}, call("getAutoWanType"), (""), 218)
    ...
    entry({"api", "xqnetwork", "set_wan"}, call("setWan"), (""), 223)
    ...
    -- AP
    entry({"api", "xqnetwork", "wifi_list"}, call("getScanList"), (""), 262)
    entry({"api", "xqnetwork", "disable_ap"}, call("disableap"), (""), 263)
    entry({"api", "xqnetwork", "mode"}, call("getMode"), (""), 264)
    entry({"api", "xqnetwork", "wan_link"}, call("getWanLinkStatus"), (""), 265)
    entry({"api", "xqnetwork", "set_wifi_ap"}, call("setWifiApMode"), (""), 266)
    entry({"api", "xqnetwork", "wifiap_signal"}, call("getWifiApSignal"), (""), 267)
    ...
    -- 协商速率
    entry({"api", "xqnetwork", "wan_speed"}, call("getWanSpeed"), (""), 272)
    entry({"api", "xqnetwork", "set_wan_speed"}, call("setWanSpeed"), (""), 273)
    -- PPPoE Catch
    entry({"api", "xqnetwork", "pppoe_catch"}, call("pppoeCatch"), (""), 274)
end
...
function setWifiTxpwr()
    ...
    XQLog.check(0, XQLog.KEY_FUNC_WIFI_SIGNAL, txpwr)
...
function setWifi()
    ...
    local wifistatus = XQWifiUtil.getWifiStatus(index)
    local shareupdate = false
    if wifistatus and wifistatus.ssid ~= ssid then
        shareupdate = true
    end
    if on ~= nil then
        on = tonumber(on)
    ...
    if code == 0 then
        LuciHttp.close()
        XQFunction.forkRestartWifi(shareupdate)
    end
end
...
function setAllWifi()
    ...
    local shareupdate = false
    if wifistatus and wifistatus.ssid ~= ssid1 then
        shareupdate = true
    end
...
function wanDown()
    luci.sys.call("env -i /sbin/ifdown wan >/dev/null 2>/dev/null")
    local result = {code=0}
    LuciHttp.write_json(result)
end
...
function setWan()
    ...
    if XQFunction.isStrNil(wanType)
        ...
        XQLog.check(0, XQLog.KEY_FUNC_WAN_METHOD, wanType)
        if wanType == "pppoe" then
            if client == "web" then
                XQLog.check(0, XQLog.KEY_VALUE_NETWORK_PPPOE, 1)
                XQLog.check(0, XQLog.KEY_FUNC_WAN_PPPOE, 1)
            end
        ...
        elseif wanType == "dhcp" then
            if client == "web" then
                ...
                XQLog.check(0, XQLog.KEY_FUNC_WAN_DHCP, 1)
            end
        ...
        elseif wanType == "static" then
            if client == "web" then
                ...
                XQLog.check(0, XQLog.KEY_FUNC_WAN_IP, 1)
            end
            local LuciDatatypes = require("luci.cbi.datatypes")
            local LuciIp = require("luci.ip")
...
function setLanDhcp()
    ...
    local ignore = tostring(LuciHttp.formvalue("ignore"))
    XQLog.check(0, XQLog.KEY_FUNC_DHCP, 1)
    if ignore and ignore == "1" then
        XQLanWanUtil.setLanDHCPService(startReq,endReq,leasetime,ignore)
    else
        local num,unit
        if leasetime and startReq and endReq then
...
-- data: [{mac: ..., upload: ..., download: ..., leave: ...}]
function qosLimits()
    local XQQoSUtil = require("xiaoqiang.util.XQQoSUtil")
    ...
    if status and status.on == 1 then
        for _, item in ipairs(data) do
            local mac      = item.mac
            local upload   = tonumber(item.upload)
            local download = tonumber(item.download)
...
function addRedirect()
    ...
    local force = tonumber(LuciHttp.formvalue("force")) == 1 and true or false
    local add = XQPortForward.setPortForward(name, ip, sport, dport, proto, force)
...
function addRangeRedirect()
    ...
    local force = tonumber(LuciHttp.formvalue("force")) == 1 and true or false
    local add = XQPortForward.setRangePortForward(name, ip, fport, tport, proto, force)
...
function addServer()
    ...
    XQLog.check(0, XQLog.KEY_FUNC_DDNS_ADD, 1)
...
function ddnsReload()
    ...
    XQLog.check(0, XQLog.KEY_FUNC_DDNS_OK, 1)
...
function getScanList()
    ...
    result["list"] = XQWifiUtil.getWifiScanlist()
    LuciHttp.write_json(result)
end
...
function disableap()
    ...
    local lanip, ssid = XQAPModule.disableWifiAPMode()
    result["ssid"] = ssid
    result["lanip"] = lanip
...
function getMode()
    ...
    elseif mode == "wifiapmode" then
        ...
        result.hostip = hostip
        result.hostname = hostname
        local apcli = XQWifiUtil.getWifiBridgedClient(1)
        if apcli then
...
function setWifiApMode()
    ...
    XQLog.check(0, XQLog.KEY_WIFI_AP, 1)
    XQLog.check(0, XQLog.KEY_FUNC_WIFI_RELAY, 1)
...
function _apSignal(step)
    ...
    local apcli = XQWifiUtil.getWifiBridgedClient(1)
    local wifilist = XQWifiUtil.getWifiScanlist(apcli.ssid)
    for _, wifi in ipairs(wifilist) do
        if wifi.ssid == apcli.ssid then
            ssid = apcli.ssid
            signal = tonumber(wifi.signal)
            break
        end
    end
    if ssid and signal then
        ...
        return _apSignal(step - 1)
    end
end

function getWifiApSignal()
    local XQFunction = require("xiaoqiang.common.XQFunction")
    local result = {
        ["code"] = 0,
        ["ssid"] = "",
...
function pppoeCatch()
    ...
    local pppoecatch = XQLanWanUtil.pppoeCatch(10)
