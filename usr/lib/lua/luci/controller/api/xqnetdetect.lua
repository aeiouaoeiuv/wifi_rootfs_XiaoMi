...
function index()
    ...
    entry({"api", "xqnetdetect", "uploadspeed"}, call("uploadSpeed"), (""), 358)
    entry({"api", "xqnetdetect", "nettb"}, call("nettb"), (""), 359)
end
...
function nettb()
    ...
    local nettb = XQSysUtil.nettb()
    if nettb.code ~= 0 then
        result["error"] = nettb.code
        result["reason"] = nettb.reason
    end
    LuciHttp.write_json(result)
end