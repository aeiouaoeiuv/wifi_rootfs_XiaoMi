...
function qosModeSwitch(auto)
    if auto then
        ...
        return os.execute("/etc/init.d/miqos set_type both") == 0
    end
end
...
function qosOnLimit(mac, maxup, maxdown, upweight, downweight)
    ...
    if mac and maxup and maxdown and upweight and downweight then
        ...
        local limit = MiQos.cmd(string.format("set_limit both %s %s %s %s %s", mac, maxup, maxdown, upweight, downweight))
