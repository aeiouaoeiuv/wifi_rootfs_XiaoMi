...
local HBASE_CONFIG_UPLOAD_URL = "https://log.miwifi.com/xiaoqiang_config/"
...
function checkUpgrade()
    ...
    local function decode(str)
        resultjson = LuciJson.decode(str)
    end
    ...
    if tonumber(resultjson["code"]) == 0 then
        ...
        if resultjson.data and resultjson.data.link then
            ...
            changeLog = XQFunction.parseEnter2br(luci.util.trim(resultjson.data.description))
            result["needUpdate"] = 1
            ...
            XQPreference.set(XQConfigs.PREF_ROM_DOWNLOAD_URL,resultjson.data.link)
        else
            local changeLog = ""
            if resultjson.data and resultjson.data.description then
                changeLog = XQFunction.parseEnter2br(luci.util.trim(resultjson.data.description))
            end
            result["needUpdate"] = 0
